...
export class GerenciadorUsuario implements OnInit{
  ...
  executarGerenciadorUsuario(): void {
    ...
    this.AuthGerenciadorUsuarioService.AuthGerenciadorUsuario(this.idSelecionado, this.nomeDigitado, this.senhaDigitada, this.emailDigitado, this.telefoneDigitado, this.cargoDigitado,usuarioAtual).subscribe({
      next: (resposta) => {
